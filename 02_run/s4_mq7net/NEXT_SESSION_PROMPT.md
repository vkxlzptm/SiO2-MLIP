...
| 0 K E-V 스캔 (자체 형성 망) | `run_ev_s4.sh` | ❌ 미실행 |
...
1. **[계산, dhl-desktop, 12분] ring 반복 2개 재실행.** 스윕이 **패치 전** `in.mqbks_fast`
   로 돌아 `prod_bks_2e13_r2/r3.data` 가 없다(로그가 "profile-only"). Tg 는 프로파일만
   쓰니 무사하지만 **ring 오차막대를 실측으로 바꾸려면 구조가 필요하다.**
   `sync.sh` 로 패치된 `in.mqbks_fast` 를 받은 뒤 두 개만 다시:
   ```
   ...
   ```
   끝나면 `ring_stats.py` 를 새 `prod_bks_2e13_r*.data` 에 돌리고 `fig_rings_s4.py` 재실행.
   → 현재 3.0σ 의 '실현간 산포'가 **추정치 → 실측치**가 된다. 가장 싼 개선.

2. **[계산, dhl-desktop, ~50분] 0 K E-V 스캔** — `run_ev_s4.sh` (2단계로 재설계됨).
   ```
   setsid nohup bash run_ev_s4.sh > ev_s4_chain.log 2>&1 < /dev/null &
   ```
   1단계 3점으로 P=0 을 실측해 V₀ 를 잡고, 2단계 7점을 **V₀ 대칭**으로 생성한다.
   BM3 피팅은 scipy 필요 → `mlip` 환경에서. 기존 값(BKS 위상 위) ρ₀ 2.2185 / K₀ 43.23 /
   K₀′ −2.02 와 뺄셈하면 **밀도에서의 위상 형성 효과**.
   BKS 짝은 `in.ev_bks` 를 `prod_bks_2e13.data` 로 (수 분).

3. **[분석, dhl-desktop] `traj_analyze.py`** — `traj_7net_mq.lammpstrj` vs
   `traj_bks_2e13.lammpstrj`. 궤적이 커서 원격 실행 후 `*_stats.dat`/`*_angles.dat`/
   `*_gr.dat` 만 sync.
   ★ 3-ring 초과의 **독립 확인 경로**: 3원환은 Si–O–Si 를 좁히므로 7net 분포에
   저각(~130° 이하) 어깨가 있어야 한다. 없으면 ring 결론을 다시 봐야 한다.

4. **[분석] S(q)** (`sq_direct.py`) — S4 궤적에 미실행. RESULTS 4절이 남긴 질문
   ("자체 melt-quench 로 위상을 다시 만들면 FSDP 가 움직이는가")에 직접 답한다.

5. **[분석, 계산 불필요] 1e14 민감도** — Tg 회귀에서 경계선(DR 2.7×)인 1e14 를 빼고
   기울기가 얼마나 움직이는지. 4.3 decade 환산의 강건성이 여기 달렸다.

6. `_to_delete/s4_chain.log` — 빈 파일, device_bash 가 못 지우니 사용자가 직접 삭제.
7. `./sync.sh` — 양쪽에서 한 번씩.
