...
**리플리카 병렬이 스레드보다 3.3배 효율적** (미검증, 착수 전 실측 필요)
OMP=6 단일 = 488.5 atom-step/s vs OMP=1 × 6개 = 1,600 atom-step/s.
RDF는 앙상블 평균이므로 "10 ps × 1개"보다 "5 ps × 6개"가 유리.
리스크: 6프로세스 × libtorch RAM(15 GB 한계), 메모리 대역폭 경합. 2~3개로 먼저 잴 것.
