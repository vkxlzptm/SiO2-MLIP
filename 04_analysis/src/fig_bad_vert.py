...
plt.rcParams.update({
    ...
    "legend.fontsize": 8.8, "legend.handlelength": 1.2, "legend.frameon": True,
...
PAN = [("O-Si-O", 2, 1, (85, 140), ((109.4, 109.7), "exp.\n109.4-109.7"), "(a)"),
...
fig, ax = plt.subplots(2, 1, figsize=(3.9, 5.7))
...
for k, (name, ocol, acol, xlim, expspec, tag) in enumerate(PAN):
    ...
    ax[0].set_yticks([0.0, 0.04, 0.08, 0.12])
    ...
    a.text(0.03, 0.88, tag, transform=a.transAxes, fontsize=11.5, fontweight="bold")
    a.text(0.145, 0.88, name, transform=a.transAxes, fontsize=11.5)

    # 실험 라벨
    xa = 0.5 * (val[0] + val[1])
    xtx = xlim[0] + (0.44 if name == "O-Si-O" else 0.72) * (xlim[1] - xlim[0])
    a.annotate(elab, xy=(xa, ymax * 0.02), xytext=(xtx, ymax * (0.26 if name == "O-Si-O" else 0.10)),
               fontsize=8.5, ha="center", va="center",
               bbox=dict(fc="white", ec="none", alpha=0.5, pad=1.0),
               arrowprops=dict(arrowstyle="-", lw=0.7, color="0.3", shrinkA=1, shrinkB=1))

ax[0].legend(loc="upper right", framealpha=0.82, borderpad=0.5) #  handlelength=1.3)
fig.suptitle(r"a-SiO$_2$,  $\rho$ = 2.20 g/cm$^3$,  300 K   "
             '\n'r"(dotted: mean;  shaded: experimental range)", fontsize=10.5, y=0.96, x=0.55)
fig.tight_layout(rect=[0, 0, 1, 1])
fig.savefig(FIG / "fig_bad_vert.png", dpi=300)
print(f"-> {FIG}/fig_bad_vert.png\n")
